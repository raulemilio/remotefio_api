...
void *gpio_output_get_thread_func(void *arg) {
    ...
    while (1) {
        if (task_queue_dequeue(&gpio_output_get_queue, &args, sizeof(ThreadGpioOutputGetDataArgs)) == 0) {
	    ...
	    gpio_output_get_running = true;
            ...
            switch (args.gpio_output_get_data->mode) {
                case 0:
                    // logica de modo 0
                    flag_index = PRU_SHD_GPIO_OUTPUT_GET_MODE0_FLAG_INDEX;
                    data_index = PRU_SHD_GPIO_OUTPUT_GET_MODE0_DATA_INDEX;
                    trigger_flag = PRU_GPIO_OUTPUT_GET_MODE0_FLAG;
                    datardy_flag = PRU_GPIO_OUTPUT_GET_MODE0_DATARDY_FLAG;
