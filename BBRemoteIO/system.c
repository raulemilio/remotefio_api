...
static uint32_t *shared; // Direccion base de la memoria
static off_t target = 0x4A310000;
//time_t start_time;
...
void *system_function(void *arg) {
    ...
    if (map_memory(&fd, &map_base, &shared, target) == -1) {
     // Si el mapeo falla, liberar el mutex y retornar
        pthread_mutex_lock(&function_mutex);
        execution_flags[function_type] = EXECUTION_ENABLED;  // Liberar el flag
        pthread_mutex_unlock(&function_mutex);
        pthread_exit(NULL);  // Abortamos el hilo si no se puede mapear la memoria
    }
