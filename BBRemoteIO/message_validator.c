...
int validate_adc_message(const char *payload, AdcData *adc_data) {
    ...
    cJSON *samples = cJSON_GetObjectItemCaseSensitive(root, JSON_KEY_SAMPLES);
    if (!cJSON_IsNumber(samples)) {
        printf(ERR_INVALID_SAMPLES);
        cJSON_Delete(root);
        return -1;
    }

    int sample_value = samples->valueint;
    if (sample_value < ADC_SAMPLES_MIN || sample_value > ADC_SAMPLES_MAX) {
        printf(ERR_SAMPLES_OUT_OF_RANGE, sample_value);
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (mode_value < ADC_MODE_MIN || mode_value > ADC_MODE_MAX) {
        printf(ERR_MODE_OUT_OF_RANGE, mode_value);
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (!cJSON_IsNumber(sample_rate_item)) {
        printf(ERR_INVALID_SAMPLE_RATE);
        cJSON_Delete(root);
        return -1;
    }

    int sample_rate_value = sample_rate_item->valueint;
    if (sample_rate_value < ADC_FS_MIN || sample_rate_value > ADC_FS_MAX) {
        printf(ERR_SAMPLE_RATE_OUT_OF_RANGE, sample_rate_value);
        cJSON_Delete(root);
        return -1;
    }
    adc_data->sample_rate = sample_rate_value;

    printf("Valid message. samples = %d, mode = %d, sample_rate = %d\n", adc_data->samples, adc_data->mode, adc_data->sample_rate);
    cJSON_Delete(root);
    return 0;
}
