...
##NODE RED
 iniciar desde el terminal node-red
 ir al servidro web http://127.0.0.1:1880/
paquetes: node-red-dashboard
 y el de led
...
```json
{ "task": "adc", "fs":1000, "resolution":4096, "vref": 1.8,
							"channels": {
...
```
**Response Topic:** `BBRemote/logs`
```json
{"task":"adc","status":"Adc finish "}
```
### GPIO INPUT
...
### GPIO OUTPUT GET
#### MODE 0:
**Command Topic:** `BBRemote/cmds/gpio/gpio_output_get`
```json
{"task":"gpio_output","output":[0,1,2,3], "mode": 0}
```
**Response Topic:** `BBRemote/rsp/gpio/gpio_output_get`
```json
{"task":"gpio_output_get","output":[0,1,2,3], "state": [0,0,0,0], "ts": 1723234234443}
```
### GPIO OUTPUT SET
#### MODE 0:
**Command Topic:** `BBRemote/cmds/gpio/gpio_output_set`
```json
{"task":"gpio_output_set","output":[0,1,2,3], "state": [0,0,0,0], "mode": 0}
```
**Response Topic:** `BBRemote/logs`
```json
{"task":"gpio_out","status":"Gpio out finish "}
```
### MOTOR GET
#### MODE 0-1-2: 
**Command Topic:** `BBRemote/cmds/motor/motor_get`
```json
{"task":"motor", "motor":[0,1,2,3], "mode": 0}
```
**Response Topic:** `BBRemote/rsp/motor/motor_get`
```json
{"task":"motor_get","motor":[0,1,2,3],"ena":[1,1,0,1],"dir":[0,1,0,1],"rpm": [60, 150, 120, 180],"step_per_rev": [200, 200, 200, 200],"micro_step": [1,1,16,2],"mode":1,"ts": 1723234234443}
```

#### MODE SET
#### MODE 0:
**Command Topic:** `BBRemote/cmds/motor/motor_set`
```json
{"task":"motor", "motor":[0,1,2,3],"ena":[1,1,1,1],"dir":[0,0,0,0],"step_time":[200000,1500,1200,1800], "mode": 3}
```
**Response Topic:** `BBRemote/logs`
```json
{"task":"motor_set","status":"Motor set finish "}
```
